...
import { fetchCarById } from "@/lib/car-service";
...
export function CarDetailModal({ carId, onClose }: { carId: string | null; onClose: () => void }) {
      ...
      {carId && (
        ...
        >
          <motion.div
            ...
            className="premium-ring relative max-h-[92vh] w-full max-w-5xl overflow-hidden rounded-t-3xl border border-border bg-card shadow-card sm:rounded-3xl"
          >
            <button
              aria-label="Close"
              onClick={onClose}
            ...
            {isLoading || error || !car ? (
              <div className="p-10 text-center">
                <div className="text-lg font-semibold">
                  {isLoading ? "Loading car details" : "Car details unavailable"}
                </div>
                <p className="mt-2 text-sm text-muted-foreground">
                  {error ? (error as Error).message : "Fetching latest details and images."}
                </p>
              </div>
            ) : (
              <div className="grid max-h-[92vh] overflow-y-auto lg:grid-cols-2">
                {/* Gallery */}
                <div className="bg-foreground p-4 dark:bg-black">
                ...
                <div className="flex flex-col gap-5 p-5 sm:p-7">
                  ...
                  <div className="grid grid-cols-2 gap-x-5 gap-y-3 rounded-2xl border border-border bg-surface p-4 text-sm">
